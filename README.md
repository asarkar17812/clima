...
11. [What the Data Doesn't See](#what-the-data-doesnt-see)
12. [Caveats](#caveats)
...
A few non-obvious cleaning steps are worth flagging. The post-2020 split of Alaska's Valdez-Cordova Census Area into Chugach (`02063`) and Copper River (`02066`) is re-merged into the pre-split-era code `02261` so the SCI (which still uses that code) joins cleanly to the ESRI demographics. The 2023 transition in Connecticut requires the inverse fix: the Census Bureau replaced the state's eight historical counties with nine Council-of-Governments planning regions as the new "county-equivalent" units, but the October 2021 SCI and the 2022 ESRI estimates both predate that transition and still use the original eight-county FIPS codes (`09001` Fairfield through `09015` Windham). When the BLS crosswalk vintage carries the new planning-region codes (`09110` through `09190`), each is remapped back to the historical county it overlaps most heavily with — Capitol Region `09110` → Hartford `09003`, South Central `09170` → New Haven `09009`, and so on — with the two planning regions that straddle more than one pre-transition county (Naugatuck Valley and Western Connecticut) assigned to the county containing the larger share of their 2020 population. The many-to-one collapse creates duplicate rows in the crosswalk, which are then de-duplicated to keep the first CBSA entry per FIPS. This keeps the crosswalk in the same eight-county Connecticut convention that the SCI and ESRI tables already use, and mirrors the Alaska step in the opposite direction. Kalawao County, HI (`15005`) is dropped because its population is in the low double digits and breaks log-scale plots. And the directed SCI dataset is kept symmetric throughout — naive unsymmetrization (deleting one of each $(i,j) / (j,i)$ pair) biases toward whichever FIPS appears first in the source dataframe, so all CBSA-level aggregates are computed using the conserved-total identity $\textbf{OCOC}_{C} = \textbf{Total}_{C} - (\textbf{ICIC}_{C} + \textbf{ICCC}_{C})$ instead.
...
Every dataset used in this notebook is, in one form or another, a surveillance product. The SCI is built from Facebook's record of who is connected to whom on a single private platform. The ESRI MAU is a model trained on commercial internet-tracking data. The 2020 Decennial Census and the ACS extracts are institutional enumerations with well-documented systematic undercounts of specific communities. The composite picture these layers compose is not the picture of *people in America*; it is the picture of *people in America who are visible to the institutions that produced these datasets*. That gap is neither random nor evenly distributed, and it is worth pulling apart before reading the figures above too literally.

The undercounts that matter most for this work cluster in a few categories. The Census Bureau's own Post-Enumeration Survey for the 2020 count documented a net undercount of roughly 3.3% for Black residents and 5.0% for Hispanic or Latino residents, against a net *overcount* of about 1.6% for non-Hispanic white residents; the undercount for residents living in non-traditional housing (basement apartments, illegally subdivided units, group quarters that are not formally classified as such) and for undocumented residents is unmeasured but, on every prior demographic study, larger than either of the above. Facebook MAU systematically excludes older adults who never adopted the platform, undocumented residents who deliberately avoid platforms that record identity to a real name, residents without reliable internet access (skewed rural and toward parts of public housing), and increasingly younger users who have moved to other platforms entirely. ESRI's MAU model is trained on this same observational layer and inherits its blind spots; what we call $|S_i|$ is the visible population of Facebook users in county $i$, not the true count, and the model behind that estimate has its own coverage geometry.

For the scaling exponents, the practical implication is that the coverage estimate $s_i = |S_i| / N_i$ is not the same *kind* of bias across all counties. Counties with larger populations in undercounted groups will see $s_i$ values that under- or over-state the true Facebook coverage in ways that don't average out geographically — both the numerator (who shows up on Facebook) and the denominator (who shows up in the Census) are shifted, and the shifts don't cancel. The *Schläpfer* rescaling we apply corrects for the simple per-county undercount; it cannot correct for the differential demographic composition of who got counted and who didn't, and so any residual bias in the scaling slopes is most plausibly toward the direction that *underweights* the communities most surveilled-but-uncounted at the same time.

For the demographic profiles of Hamilton Beach, Howard Beach, and Red Hook, the implication is that Figures 8 through 14 are showing not the demographics of those communities, but the demographics *as the 2020 Census recorded them*. The Hispanic / Latino share in Hamilton Beach in particular — the largest single shift in Figure 9 relative to the citywide baseline — is most likely *higher* than the 36% shown, given the documented Hispanic undercount; the reported share is a floor, not a point estimate. The same point applies to the renter-occupied bar in Figure 12, which is sensitive to under-enumeration of informal rental arrangements that are widespread in the very flood-zone neighborhoods this project studies.

For the flood-risk modeling that all of this feeds into, the gap is consequential in a specific and asymmetric direction. The same demographic groups most underrepresented in the data are often those most exposed to coastal flood risk in NYC: undocumented residents in basement units in flood-zone neighborhoods, elderly residents who own their homes outright but never appear in any MAU panel, and public-housing residents whose Census participation rates historically trail the borough average. A model that takes these datasets at face value will be calibrated to what the data shows; the population at greatest risk is, in practice, calibrated *under* that. The reporting rates of damage and displacement after the actual flood event will then follow the same surveillance pattern, in a feedback loop that the model can absorb but cannot correct on its own.

Naming the gap doesn't invalidate the results, but it does locate them. The scaling exponents and the demographic figures are valid descriptions of a particular slice of reality — the slice that is legible to the institutions that produce the underlying datasets. They are useful starting points for understanding coastal flood vulnerability, not endpoints. The qualitative interview material in the previous section is, in part, a deliberate attempt to bring in some of what the quantitative layer cannot see; the most honest reading of this notebook is one that treats the quantitative results as a tractable approximation rather than the whole story, and pairs them with whatever community-level ground-truthing is available before any policy claim is built on top.
